...
// if unable to convert AppErrCode, throw with original error response
            throw new AppException(ErrorCode.SERVER, e2.getCause());
        }
    }
    ...
    public  <T> List<T> requestObject(String uri, Class< T> type, boolean flag) throws Exception { this.uri = uri; return requestObject(type, flag); }

    public  <T>  List<T> requestObject(Class<T> type, boolean flag) throws Exception {
        return JsonConverter.toList(EntityUtils.toString(requestEntity()), type);
    }
